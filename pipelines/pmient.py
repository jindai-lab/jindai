...
class PMILREntropyWordFetcher(NgramCounter):
    ...
    def __init__(self, word_length=4, min_pmi=2, min_lr_ent=1, min_freq=1e-5, **kwargs):
        """
        Args:
            word_length (int, optional): 抽取的最大词长度。
            min_pmi (float, optional): 最小互信息熵阈值。
            min_lr_ent (float, optional): 最小左右信息熵阈值，若为0则将跳过。
            min_freq (float, optional): 最小词频。
        """
    ...
    def resolve(self, p : Paragraph):
        self.text_length[p.id] = len(p.content)
        super().resolve(p)
    ...
    def summarize(self, returned):
        ...
        if self.min_lr_ent > 0: 
            r = self.ent_lr()
        return sorted([(w, self.ngrams[w]) for w in r], key=lambda x: x[1], reverse=True)
            
    def pmient(self):
        candidates = []
        min_p = 2 ** self.min_pmi / self.text_length
        min_freq = int(self.min_freq * self.text_length)
        print('generating pmient words...')
        ...
        for word in list(self.ngrams):
            if 1 < len(word) <= self.word_length and self.ngrams[word] > min_freq \
                ...
                px_py = min([self.ngrams.get(word[:i], 1) * self.ngrams.get(word[i:], 1) for i in range(1,len(word))])
