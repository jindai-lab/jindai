...
def download_dataset(host, count, dataset_dir):
    ...
    positives = requests.post(f'http://{host}/get', headers={'content-type': 'application/json'}, data=json.dumps({'limit': int(count * 1.1), 'tag': '_rating>1'})).json()['results']
    negatives = requests.post(f'http://{host}/get', headers={'content-type': 'application/json'}, data=json.dumps({'limit': int(count * 1.1), 'tag': '_rating<=-1|(@asiantolick,_rating=0)'})).json()['results']
