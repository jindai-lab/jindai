...
def worker():
    while True:
        ...
        try:
            ...
            # 任务成功，保存结果
            if result is None:
                print(f'Result for {task_id} empty, removed')
                r.delete(res_key)
            else:
                r.hset(res_key, mapping={
                    "status": "completed",
...
def handle_embedding(id = None, content = None, bulk = None):
    ...
    if bulk is not None:
        ...
        for i in bulk:
            id = i['id']
            content = i['content']
            emb = TextEmbeddings(id=id, embedding=TextEmbeddings.get_embedding(content))
            embs.append(emb)
        ...
        stmt = (
            select(Paragraph.id, Paragraph.content)
            .where(
                ~exists()
