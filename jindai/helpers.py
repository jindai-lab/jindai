...
class APICrudEndpoint:
    ...
    def build_query(self, id, ids, query, data):
        ...
        # query id name or id
        if id:
            query = F.id == id if re.match(
                r'^[0-9a-fA-F]{24}$', id) else F.name == id
    ...
    def update_object(self, obj, data):
        ...
        for field, newval in data.items():
            ...
            if isinstance(newval, dict) and [1 for k in newval if k.startswith('$')]:
                ...
                if '$round' in newval:
                    obj[field] = round(
                        obj[field] / newval['$round']) * newval['$round']
                    newval.pop('$round')
    ...
    def bind(self, app: Flask, **options):
        ...
        app.add_url_rule(f'{self.namespace}', methods=[
                         'GET', 'PUT', 'POST'], view_func=do_crud, endpoint=endpoint)
        app.add_url_rule(f'{self.namespace}<id>', methods=[
                         'GET', 'POST', 'DELETE'], view_func=do_crud, endpoint=endpoint)
        ...
        app.add_url_rule(f'{self.namespace}scheme', methods=[
                         'GET'], view_func=get_scheme, endpoint=endpoint + '_scheme')

        # Add url rule for all maps in the map.
        for func_name, func in self.maps.items():
            app.add_url_rule(f'{self.namespace}{func_name}', methods=[
                             'GET', 'POST'], view_func=rest(**options)(func), endpoint=endpoint + '_' + func_name)
